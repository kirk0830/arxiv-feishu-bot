...
run at **09:30 Beijing Time (01:30 UTC)** every day.
...
Change the category clause from:
...
```python
f'(cat:physics.chem-ph OR cat:cond-mat.mtrl-sci '
f'OR cat:physics.comp-ph OR cat:physics.bio-ph '
f'OR cat:cond-mat.stat-mech) '
```

#### Example: Add Force Field and Coarse-Grained Keywords

Change the keyword clause from:
...
f'AND (all:"molecular dynamics" OR all:"machine learning" '
f'OR all:"deep learning" OR all:"neural network" '
f'OR all:"force field" OR all:"coarse-grained" '
f'OR all:"neural network potential") '
```

#### Example: Search Only Titles (Stricter)

Replace `all:` with `ti:` to restrict matches to paper titles only:

```python
f'AND (ti:"molecular dynamics" OR ti:"machine learning") '
```

#### Example: Extend Date Range to Weekly Digest

Modify the date calculation in `fetch_papers_from_arxiv()`:

```python
yesterday = datetime.utcnow() - timedelta(days=7)  # Last 7 days
```
